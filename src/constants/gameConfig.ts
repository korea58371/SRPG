...
export function getTileDarkness(x: number, y: number, w: number, h: number): number {
  ...
  // 거리가 0.65 이하인 중앙 뷰 영역은 이동 가능 구역(안개 0) - 가로/세로 기준 정확히 중심 65% 면적
  if (dist <= 0.65) return 0;

  // 0.65 부터 1.0(가장자리)까지 부드러운 그라데이션 안개
  const factor = 0.2 + ((dist - 0.65) / 0.35) * 0.8;
...
export const UNIT_CONFIG = {
  PLAYER_UNIT_COUNT: 10,        // 아군 최대 출격 수
  ENEMY_UNIT_COUNT: 10,         // 적군 스폰 수
  HERO_UNIT_COUNT: 3,           // 지휘관
  BASE_SPEED_MIN: 2,
  BASE_SPEED_MAX: 3,
...
// 게임의 기준 턴별 기준년도 (1턴 = 1개월)
export const BASE_YEAR = 1999; 
...
export const TERRAIN_BONUS = {
  ...
  [TerrainType.DESERT]: { defenseMod: -0.1, moveCost: 1.5 }, // 사막: 방어력 약간 감소, 이동 패널티
  [TerrainType.SNOW]: { defenseMod: 0.1, moveCost: 2.0 },    // 설원: 방어력 약간 상승, 이동 패널티
};
...
// ─ 병종별 기본 스킬 ───────────────────────────────────────────────
export const TROOP_SKILLS: Record<string, string[]> = {
  INFANTRY: ['mock-ambush'], 
  SPEARMAN: ['mock-push', 'mock-line'],
  CAVALRY:  ['mock-dash-attack'],
  ARCHER:   ['mock-radius', 'mock-poison'],
  GENERAL:  ['mock-buff-atk', 'mock-heal'],
};
...
export const GENERAL_INITIAL_STATS = {
  senior: { power: 40, toughness: 30, constitution: 60, agility: 5, command: 35, leadership: 40, intelligence: 35, politics: 30, charm: 5 }, // 주장
  junior: { power: 25, toughness: 20, constitution: 40, agility: 4, command: 20, leadership: 25, intelligence: 20, politics: 25, charm: 4 }, // 부장國
};
